...
- [x] 用真实浏览器验收 `/inventory`：桌面 1366x900/1440x900、手机 390x844。
- [x] operator 登录后验证：库位地图、库位详情、调拨、对账 preview、拣货推荐都可用。
- [x] supervisor 登录后验证：数量校正面板可见，operator 不可见。
- [x] 验证多物料库位：调拨前必须选择具体物料，不允许含糊移动整个库位。
- [x] 验证超量调拨：现场数量大于系统数量时，系统允许调拨并写盘点差异流水。
- [x] 验证临时库位清空后隐藏，永久库位清空后保留并提示补货。
- [x] 检查浏览器 console error、移动端按钮遮挡、表格/卡片文字溢出、滚动可用性。
- [ ] 修复 QA 发现的问题后补静态 HTML/API 测试，必要时补 Playwright smoke。
- [x] 2026-05-12 P0-2 `/inventory` Playwright QA 17/17 通过：operator/supervisor、桌面/手机、调拨、超量调拨、对账 preview、拣货推荐和布局检查均通过，未发现需修复缺陷。
- [x] 2026-05-12 P0 空库位规则 API 验收通过：operator 调拨清空临时库位后默认列表和地图隐藏，`include_hidden=true` 可追溯；调拨清空永久库位后仍在列表和地图显示，并标记 `restock_required`。
