...
$ExcludeDirs = @(
    ...
    'build', 'dist', 'logs', 'tagledger.egg-info', '__pycache__',
...
try {
    foreach ($Entry in $Archive.Entries) {
        ...
        if (
            ...
            $Lower -match '(^|/)docs/private/' -or
            $Lower -match '(^|/)docs/ops/private/' -or
            $Lower -match '(^|/)private_requirements/' -or
...
Write-Host $ZipPath
Write-Host ''
Write-Host 'Sanity check passed. Sanitized package excludes data\app.db, data\uploads, data\screenshots, logs, docs\private, .env, .git and .omx.'
