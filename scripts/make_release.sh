...
rsync -a \
  ...
  --exclude 'docs/ops/private/' \
  --exclude 'PRIVATE_REQUIREMENTS/' \
...
if forbidden_entries=$(unzip -Z1 "$ZIP_PATH" | grep -E '(^|/)(\.git|\.omx)/|(^|/)data/app\.db|(^|/)data/storage_state\.json|(^|/)data/[^/]+\.xlsx|(^|/)data/outbound/|(^|/)data/ocr-scratch/|(^|/)data/uploads/.+\.(jpg|jpeg|png|webp)|(^|/)data/screenshots/.+\.(png|jpg|jpeg|txt)|(^|/)logs/.+\.log|(^|/)docs/private/|(^|/)docs/ops/private/|(^|/)PRIVATE_REQUIREMENTS/|(^|/)\.env($|\.)'); then
